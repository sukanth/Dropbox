...
/**
 * Computes a hash using the same algorithm that the Dropbox API uses for the the "content_hash"
 * metadata field.
 *
 * <p>The {@link #digest()} method returns a raw binary representation of the hash. The
 * "content_hash" field in the Dropbox API is a hexadecimal-encoded version of the digest.
 *
 * <p>Example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>If you need to hash something as it passes through a stream, you can use the {@link
 * java.security.DigestInputStream} or {@code java.security.DigestOutputStream} helpers.
 *
 * <pre>
 * MessageDigest hasher = new DropboxContentHasher();
 * InputStream in = new FileInputStream("some-file");
...
public final class DropboxContentHasher extends MessageDigest implements Cloneable {
  ...
  private DropboxContentHasher(
      MessageDigest overallHasher, MessageDigest blockHasher, int blockPos) {
